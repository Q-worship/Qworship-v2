...
import { AuthApiError, persistAuthSession, signIn, getPostAuthPath } from '@/lib/authApi'
...
export function LoginForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (err instanceof AuthApiError && err.errorType === 'EMAIL_NOT_VERIFIED') {
        const pendingEmail = err.email || email.trim().toLowerCase()
        sessionStorage.setItem('verifyEmail', pendingEmail)
        setLocation(`/verify?email=${encodeURIComponent(pendingEmail)}`)
        return
      }
    ...
    <div className="login-form">
      ...
      <form className="login-form__fields" onSubmit={handleSubmit}>
        ...
        <div className="login-form__options">
          ...
          <Link href="/forgot-password" className="login-form__link">Forgot Password</Link>
        </div>
