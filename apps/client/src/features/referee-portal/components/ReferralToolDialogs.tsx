...
function QRCodeDialog({ open, onClose, referralLink, referralCode }: Omit<Props, "active"> & { open: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={(value) => !value && onClose()}>
    <DialogContent className="max-h-[92vh] overflow-y-auto rounded-[26px] p-0 sm:max-w-[920px]">
...
function ShareMessageDialog({ open, onClose, referralLink, referralCode }: Omit<Props, "active"> & { open: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={(value) => !value && onClose()}>
    <DialogContent className="max-h-[94vh] overflow-y-auto rounded-[26px] sm:max-w-[1080px]">
...
function CampaignLinkDialog({ open, onClose, referralLink, referralCode }: Omit<Props, "active"> & { open: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={(value) => { if (!value) { onClose(); window.setTimeout(reset, 250); } }}>
    <DialogContent className="max-h-[94vh] overflow-y-auto rounded-[26px] sm:max-w-[1040px]">
