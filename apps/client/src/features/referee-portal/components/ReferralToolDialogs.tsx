      ...
      <div className="grid lg:grid-cols-[.92fr_1.08fr]">
        <div className="relative grid min-h-[390px] place-items-center overflow-hidden rounded-t-[26px] bg-[#eeeafd] p-8 lg:rounded-l-[26px] lg:rounded-tr-none">
          ...
          <div className={`relative w-full max-w-[330px] rounded-[26px] p-5 shadow-[0_22px_60px_rgba(52,39,91,.16)] transition-colors ${theme === "dark" ? "bg-[#282330]" : "bg-white"}`}>
            ...
            <div className={`aspect-square overflow-hidden rounded-[18px] p-3 ${theme === "dark" ? "bg-[#282330]" : "bg-white"}`}>{generating ? <div className="grid h-full place-items-center text-xs text-[#716a79]">Generating secure QR…</div> : <img src={dataUrl} alt={`QR code for ${referralLink}`} className="h-full w-full object-contain"/>}</div>
            {includeLabel && <div className="mt-4 text-center"><div className={`text-sm font-extrabold ${theme === "dark" ? "text-white" : "text-[#27232e]"}`}>Scan to explore Q-Worship</div><div className={`mt-1 truncate text-[10px] ${theme === "dark" ? "text-[#b9b1c3]" : "text-[#716a79]"}`}>{referralLink}</div></div>}
          </div>
        </div>
        <div className="p-6 sm:p-8">
          ...
          <div className="mt-6 space-y-5">
            ...
            <div className="flex items-center justify-between rounded-xl border border-[#ebe7f1] p-4"><div><div className="flex items-center gap-2 text-sm font-bold"><Palette size={15} className="text-[#8054F6]"/>Include branded label</div><p className="mt-1 text-xs text-[#716a79]">Adds the Q-Worship name and your code below PNG exports.</p></div><Switch checked={includeLabel} onCheckedChange={setIncludeLabel}/></div>
