...
export function ReferralsView() {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <AlertDialog open={!!detailReferee} onOpenChange={(open) => !open && setDetailReferee(null)}>
        <AlertDialogContent className="sm:max-w-lg">
          <AlertDialogHeader>
            <AlertDialogTitle>{detailReferee?.firstName} {detailReferee?.lastName}</AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="space-y-2 text-left text-sm">
                <p><span className="font-semibold text-gray-700 dark:text-gray-300">Email:</span> {detailReferee?.email}</p>
                <p><span className="font-semibold text-gray-700 dark:text-gray-300">Phone:</span> {detailReferee?.phoneNumber || '—'}</p>
                <p><span className="font-semibold text-gray-700 dark:text-gray-300">Country:</span> {detailReferee?.countryCode || '—'}</p>
                <p><span className="font-semibold text-gray-700 dark:text-gray-300">Joined:</span> {detailReferee ? new Date(detailReferee.createdAt).toLocaleDateString() : ''}</p>
                <p><span className="font-semibold text-gray-700 dark:text-gray-300">Last sign-in:</span> {detailReferee?.lastLogin ? new Date(detailReferee.lastLogin).toLocaleDateString() : 'Never'}</p>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
