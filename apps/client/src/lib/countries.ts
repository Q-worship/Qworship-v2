...
const PRIORITY_COUNTRY_CODES = ['GB', 'US', 'CA', 'AU', 'NG', 'GH', 'ZA', 'KE'] as const
...
const allCountryNames = ISO_COUNTRY_CODES
  .map(codeToName)
  .filter((name): name is string => Boolean(name))
  .sort((a, b) => a.localeCompare(b))

const priorityCountryNames = PRIORITY_COUNTRY_CODES
  .map(codeToName)
  .filter((name): name is string => Boolean(name))
...
export const COUNTRY_OPTIONS: string[] = [
  ...
  ...allCountryNames.filter((name) => !prioritySet.has(name)),
]

export const PRIORITY_COUNTRY_COUNT = priorityCountryNames.length

const DIAL_CODES: Partial<Record<(typeof ISO_COUNTRY_CODES)[number], string>> = {
...
export const COUNTRY_NAME_TO_DIAL_CODE: Record<string, string> = Object.fromEntries(
  ISO_COUNTRY_CODES
    .map((code) => {
      const name = codeToName(code)
      const dialCode = DIAL_CODES[code]
      return name && dialCode ? [name, dialCode] : null
    })
