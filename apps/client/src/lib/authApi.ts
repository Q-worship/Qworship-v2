...
export class AuthApiError extends Error {
  constructor(message: string, public errorType?: string, public email?: string) {
    super(message)
...
async function authFetch<T>(
  ...
  try {
    ...
    let data: T & { message?: string; errorType?: string; email?: string }
    try {
    ...
    if (!response.ok) {
      throw new AuthApiError(data.message ?? `Request failed (${response.status})`, data.errorType, data.email)
    }
...
export function persistAuthSession(token: string, user?: AuthUser): void {
  ...
  useAuthStore.getState().setAuth(user as any)
}
...
export function getPostAuthPath(): string {
  ...
  return user?.onboardingStatus === 'completed' ? '/project-selection' : '/onboarding'
}
...
export interface ChurchInfoPayload {
  churchName: string
  denomination: string
  country: string
  city: string
...
export interface OnboardingStateResponse {
  success: boolean
  onboarding: {
    status: AuthUser['onboardingStatus']
    selectedFeatures: string[]
    organization?: { name: string; denomination?: string; country?: string; city?: string; zipCode?: string }
    trialStatus?: AuthUser['trialStatus']
    trialStartDate?: string
    trialEndDate?: string
  }
}
...
export function getCurrentUser() {
  return authFetch<{ success: boolean; user: AuthUser }>('/auth/me', { method: 'GET', headers: bearerHeaders() })
}

export function getOnboardingState() {
  return authFetch<OnboardingStateResponse>('/onboarding', { method: 'GET', headers: bearerHeaders() })
}

export function saveOnboardingOrganization(payload: ChurchInfoPayload) {
...
export function completeOnboarding() {
  return authFetch<{ success: boolean; nextStep: string; trial: { status: string; startedAt: string; endsAt: string; daysRemaining: number } }>('/onboarding/complete', { method: 'POST', headers: bearerHeaders() })
}

export function requestPasswordReset(email: string) {
  return authFetch<{ success: boolean; message: string }>('/auth/forgot-password', { method: 'POST', body: JSON.stringify({ email: email.trim().toLowerCase() }) })
}
